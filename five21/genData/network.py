...
class ResNet(object):
    ...
    def network2(self):
        ...
        with tf.variable_scope("bone"):
            ...
            f = tf.layers.conv2d(f, 64, 3, padding="SAME", data_format=DATA_FORMAT, name="conv2", activation=None,
                                 use_bias=False)
            f = tf.layers.batch_normalization(f, axis=1, training=self.training, name="bn1")
            f = tf.nn.elu(f)
            f = tf.layers.conv2d(f, 128, 3, padding="SAME", data_format=DATA_FORMAT, name="conv3", activation=None,
                                 use_bias=False)
            f = tf.layers.batch_normalization(f, axis=1, training=self.training, name="bn2")
            f = tf.nn.elu(f)

        with tf.variable_scope("value"):
            v = tf.layers.conv2d(f, 32, 1, padding="VALID", data_format=DATA_FORMAT, name="conv1", activation=None,
                                 use_bias=False)
        ...
        with tf.variable_scope("policy"):
            p = tf.layers.conv2d(f, 32, 1, padding="VALID", data_format=DATA_FORMAT, name="conv1", activation=None,
                                 use_bias=False)
