...
class Player(object):
    def __init__(self, cfg=None, training=True, pipe=None):
        self.config = config
    ...
    def evaluate_and_expand(self, state: str, board: np.ndarray = None):
        ...
        data_to_send = utils.board_to_inputs(board)  # 在接收端去创建batch维度
        ...
        # policy = np.ones((1, self.config.board_size* self.config.board_size))
        # value = np.array([0.1])
