...
def main(game_file_saved_dict="game_record", restore=False):
    ...
    e = config.noise_eps
    delta_e = (config.noise_eps - config.final_eps) / config.total_step
    while step < config.total_step:
        net.sess.run(tf.assign(lr, config.get_lr(step)))
        print("")
        time1 = int(time.time())
        if e >= config.final_eps:
            e -= delta_e
        ...
        stack.push(game_record, white_win, count_black=count_black, count_white=count_white)
        if stack.is_full():  # 满了再训练太慢了，但是消除了biase
        # if len(stack.data) > config.batch_size:
            for _ in range(4):
