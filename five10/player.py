...
class Action(object):
    def __init__(self):
        self.n = 1  # 初始化为1，使得概率分布smooth一丢丢
...
class Player(object):
    ...
    def select_action_q_and_u(self, state: str) -> tuple:
        ...
        # 对于根节点，保证每个结点至少被访问一次(访问初始化为1了，所以这里<=1)。
        # 故要求simulation_per_step >> board_size*board_size才有意义
        # 这么做使得概率分布更加smooth，并且探索得更好
        if self.root_state == state:
            no_visits = np.where(counts <= 1)[0]
            if no_visits.shape[0] > 0:
                act_idx = np.random.choice(no_visits)
                return action_keys[act_idx]
        # max_q = q_value.max()
        # 动作访问次数的初始值为1，第一次更新的最大q值是0.5，若有超过0.99的，必定是很牛逼的
        # if max_q > 0.999:
        #     act_idx = np.random.choice([idx for idx in range(act_count) if q_value[idx] == max_q])
        #     action = action_keys[act_idx]
        #     print("get a max q value %0.3f at {}, n: %d, p: %0.3f, q: %0.3f".format(action) % (
        #         max_q, node.a[action].n, node.a[action].p, node.a[action].q))
        #     return action
