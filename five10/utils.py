...
class RandomStack(object):
    ...
    def push(self, data: list, white_win, count_black, count_white):
        ...
        # 先手必赢，故加上下面这句话，使得增强后手赢棋的数据量, ，elif也是为了防止走另一个极端
        # 貌似下面的这个1.1需要进一步调小一点，例如1.04
        if white_win and random.random() < (count_black - count_white) / (count_white * 1.1):
            print("push a copy for white")
            self.data.extend(data)
        elif not white_win and random.random() < (count_white - count_black) / (count_black * 1.5):
