# -*- coding: utf-8 -*-
board_size = 11  # 棋盘大小
buffer_size = 8000  # 棋盘越大，这个数字就得越大
...
ckpt_path = "ckpt"
total_step = 20000
tau_decay_rate = 0.94  # 温度衰减项，越往后温度应该越低  0.94^10*1.86=1.0，即初始温度为1.86时，在10步以后温度衰减为1.0
# 噪声项
noise_eps = 0.2
final_eps = 0.1
c_puct = 5.0  # 这个数调大一点，使得探索得更好，每个动作的访问次数分布得越均匀，初始温度就可以调的越低
dirichlet_alpha = 0.3  # 这个值越大，产生的分布越均匀，棋盘越大，这个值也相应稍微调大点为好
gamma = 0.9  # 权重衰减因子，越是后面的局面，出现的频率就越低，分配的训练权重越大
init_temp = 1.86  # 初始温度  # 温度越大分布越平缓，这里适当调大，主要是考虑到初始分布太尖锐了
